...
| 6 | MED | *(fixed, Program-Fix 12)* The outbox drain clock was a 5-minute GitHub Actions cron on a personal repo — best-effort scheduling (measured at ~3 h), auto-disables after 60 days of inactivity. Now a Vercel per-minute cron with the hourly Actions run as backup. | `vercel.json`, `.github/workflows/worker-heartbeat.yml` |
