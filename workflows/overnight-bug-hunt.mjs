...
const pr = await agent(
  'Apply these CONFIRMED bug fixes and open ONE pull request (overnight, unattended). For EACH flaw: first write a FAILING regression test reproducing it, then the minimal fix, so the test goes red->green. Do NOT weaken or skip any existing test. Then run the full gate and it MUST pass: ' +
