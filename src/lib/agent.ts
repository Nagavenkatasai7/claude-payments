 ...
 * which would otherwise be stripped from every reply. Never throws. Pure.
 */
export function replyAllowHosts(appBaseUrl: string, brand?: string): string[] {
  const hosts: string[] = [];
  try {
    const h = new URL(appBaseUrl).hostname.toLowerCase();
    if (h) hosts.push(h);
  } catch {
 ...
 * R6b: a bare domain ("pay-now.example") is stripped too, unless its host is
 * in `allowHosts`. Links are appended AFTER the strip, so they are never cut.
 */
export function sanitizeReply(
  reply: string,
  paymentLinks: string[],
  allowHosts: readonly string[] = replyAllowHosts(env.appBaseUrl),
): string {
  // Strip every URL the model wrote — the model must never emit links itself.
  const stripped = stripModelHosts(reply.replace(/https?:\/\/\S+/gi, ''), allowHosts)
    .replace(/[ \t]+/g, ' ')
    .replace(/ *\n/g, '\n')
    .trim();
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      if (assistant.tool_calls && assistant.tool_calls.length > 0) {
        for (const call of assistant.tool_calls) {
          ...
          // Collect canonical payment links generated by our code. Web only
          // (R6b): WhatsApp's dispatch refuses generate_payment_link, and this
          // channel check keeps a link off WhatsApp structurally, like pay_url.
          if (
            channel === 'web' &&
