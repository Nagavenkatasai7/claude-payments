...
- Agent launches, subagent finishes and `gh pr merge|close` commands are journaled by hooks automatically (`.claude/hooks/ledger-journal.mjs`). The Agent prompt is never logged.
...
- `PostToolUse` matcher `Agent` and matcher `Bash`, and `SubagentStop` → `ledger-journal.mjs`. Main thread only: input with `agent_id` (a subagent's own tool call) is skipped. Always exits 0.
- `Stop` → `ledger-sync-due.mjs` (beside the tsc/eslint/vitest gate). It blocks once (never while `stop_hook_active`) when the journal is longer than the `flushed` marker, or when `git ls-remote origin main` (3 s timeout; on error it does not block) differs from `last-sync.json`. **Cloud safety:** it never blocks when `CLAUDE_CODE_REMOTE` is set (Claude Code sets it to `"true"` in remote/web sessions, where the cloud routine runs), and `~/.smartremit-ledger` is created lazily. `LEDGER_SYNC_HOOK=off` disables it by hand.
