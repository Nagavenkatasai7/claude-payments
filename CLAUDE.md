...
- **Rolling Releases (prod)**: every production deploy serves 10% of traffic for 5 min, then auto-promotes to 100% (Skew Protection on). So old and new code overlap for **at least 5 min after every production deploy**: a migration or contract change must work for both builds. `smoke.yml` waits for 100% before testing; to check by hand, `curl -s 'https://smartremit.ai/api/version?vcrrForceStable=true'` shows the new SHA only at 100% (a plain curl is sticky per client).
