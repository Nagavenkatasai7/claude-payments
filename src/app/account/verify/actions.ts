 ...
 * Gated by requireCustomer; creates a real Persona inquiry (reference-id = the
 * customer's phone — the spine that ties the webhook back to this account),
 * records `inquiry_started` + the inquiry id (audit-logged), then redirects to
 * the Persona hosted flow where raw PII is captured (never on our servers).
 */
