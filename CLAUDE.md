...
- **Rolling Releases (prod)**: every production deploy serves 10% of traffic for 5 min, then auto-promotes to 100% (Skew Protection on). So old and new code overlap for **at least 5 min after every production deploy**: a migration or contract change must work for both builds. `smoke.yml` waits for 100% before testing; to check by hand, `curl -s 'https://smartremit.ai/api/version?vcrrForceStable=true'` shows the new SHA only at 100% (a plain curl is sticky per client). **Never merge the next PR to main while a rolling release is active**: wait for the previous merge's smoke, whose wait step passing means 100%. Otherwise the new deploy is built but NOT promoted (Vercel starts no new rollout until the active one resolves).
