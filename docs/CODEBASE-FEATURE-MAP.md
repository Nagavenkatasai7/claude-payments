...
|---|---|---|---|
| Agent loop (completeTurn) — multi-round tool dispatch (MAX_TOOL_ROUNDS=6) | ✅ built | src/lib/agent.ts `completeTurn`; `MAX_TOOL_ROUNDS` = 6; `executeTool` dispatch inside the round loop (anchors by symbol, re-verified 2026-09-22) | Core loop: history + injected notes -> LLM -> tool calls -> results -> append -> repeat, up to 6 rounds/turn. |
...
| Recent-transfer memory (get_customer_context, fix 5) | ✅ built | src/lib/agent.ts (round-0 synthetic pair); buildCustomerContext in src/lib/tools.ts; getRecentTransfers in src/lib/recent-transfers.ts | Top 5 most-recent transfers (names clamped) + the tapped saved recipient, injected once at round 0 as a get_customer_context tool call + result (never a system message, never persisted); nothing for history-less customers. |
...
| Async agent turn execution (durable `agent.turn` outbox row, fast webhook ack) | ✅ built | src/lib/whatsapp-inbound.ts `processInboundWebhook` enqueues `agent.turn` (wamid-deduped) + `pokeWorker`; src/lib/outbox-worker.ts runs it (re-verified 2026-09-22) | Returns {ok:true} before the agent runs; the turn survives a killed function and is retried by the worker, not an `after()` hook. |
...
| Two-stage mock delivery (stage1 paid/charge, stage2 delivered after 120s via a delayed outbox row) | ✅ built | src/lib/settlement.ts `beginSettlement` enqueues `mocksettle:<id>` with `delayMs: DELIVERY_DELAY_MS`; `DELIVERY_DELAY_MS` = 120000 in src/lib/providers/payment-provider.ts (re-verified 2026-09-22) | Stage 1 commits in one transaction with the paid flip; stage 2 is drained by the worker, not an `after()` sleep; providerRef='mock-<id>'. |
...
| Login brute-force throttle (reserve-before-compare: 10/h per phone+IP, 30/day per phone, 50/h per IP; fail-closed) | ✅ built | src/lib/customer-auth-store.ts `reserveLoginAttempt`/`clearLoginFailures`; account/actions.ts `loginAction`/`changePasswordAction`/`resetAction` | Attempt reserved atomically before the Argon2 verify (fix 19); a stranger from one IP cannot lock the owner out; a login or a WhatsApp-proven reset unlocks. |
...
| Customer detail page (identity/KYC, transfers, verify/reject actions) | ✅ built | src/app/admin-dashboard/customers/[ref]/page.tsx; manualKycDecisionAction/reviewKycAction | Every KYC decision needs a reason and writes a durable audit_events row (Program-Fix 28); no one-click override. |
...
| Proxy (ex-middleware) page-level gating (/admin-dashboard and /account require a session cookie) | ✅ built | src/proxy.ts | Layered defense: page redirect + endpoint require* gates. |
...
| Smoke test (Playwright E2E on prod after deploy) | ✅ built | .github/workflows/smoke.yml; on push to main (waits for the Vercel build + rolling release), workflow_dispatch `sha`, and deployment_status success Production (defers to the push run) | BASE_URL canonical alias bypasses Vercel protection; uploads trace on failure (7 days). |
